...
export class PopoverContentComponent implements AfterViewInit, OnDestroy {
    ...
    @Input() public closeOnClickInside = false;
    ...
    public onCloseFromInside = new EventEmitter();
    ...
    /**
     * Closes dropdown if user clicks inside this directive.
     */
    public onPopoverMouseDown = (event: any) => {
        const element = this.element.nativeElement;
        if (element.contains(event.target)) {
            this.onCloseFromInside.emit(undefined);
        }
    }
    ...
    public ngAfterViewInit(): void {
        ...
        if (this.closeOnClickInside) {
            this.listenClickFunc = this.renderer.listen('document', 'mousedown', (event: any) => this.onPopoverMouseDown(event));
        }
    ...
    protected positionElements(hostEl: HTMLElement, targetEl: HTMLElement, positionStr: PopoverPlacement, appendToBody: boolean = false): { top: number, left: number } {
        ...
        const pos1 = positionStrParts[1] || 'top';
